...
class ReservaCreationSerializer(serializers.ModelSerializer):
    ...
    taxista_id = serializers.PrimaryKeyRelatedField(
        write_only=True, queryset=User.objects.all(), source='taxista', allow_null=True)
    confirmada = serializers.BooleanField(default=False)
    hora_creada_reserva = serializers.TimeField(default=now)

    class Meta:
        model = Reserva
        fields = ['id', 'nombre', 'email', 'direccion', 'hora_recogida',
                  'telefono', 'taxista', 'taxista_id', 'confirmada', 'hora_creada_reserva']
